    ...
    <div class="row pt-2">
        ...
        <?php if (!empty($subject)){ foreach ($subject as $sub){ $totalExamPending = get_subID_by_exam($sub->subject_id) - get_subID_by_done_exam($sub->subject_id); ?>
            <div class="col-6 cl-p-r" >
                <a href="<?php echo base_url()?>/Mobile_app/Quiz/exam/<?php echo $sub->subject_id?>" class="btn d-block btn-cl position-relative" ><?php echo $sub->name;?> <span class="position-absolute  badge bg-secondary rounded-pill top-0 bg-success"><?php echo $totalExamPending > 0 ? $totalExamPending : 0;?></span></a>
            </div>
